...
public class PKOTest extends MainCashConfigTest {
	...
	@Test(  enabled = false,
			description = "SRTE-175. Добавление нового документа Приходной кассовый ордер (ПКО) на вкладку \"Документы\" Главной кассы. Проверка изменения баланса при добавлении", 
			dataProvider = "PKO")
	public void testAddNewPKO(String docType){
		docsOnPage = docs.getDocCountOnPage();
		balanceStart = docs.getBalance(BALANCE_END);
		...
		Assert.assertEquals(docs.getBalance(BALANCE_END), balanceStart.add(new BigDecimal(docSum.replace(",", "."))), "Документ РКО " + docType.toString() + ": не изменился баланс после добавления документа");
	}
	...
	@Test(enabled = false, description = "SRTE-175. Сохранение полей ПКО при редактировании и нажатии кнопки \"Сохранить изменения\"")
	public void testSavedFieldsPKO(){
	}
